...
window.addEventListener('load', function () {
  function addOtherPrice() {
    ...
    const productId = document.getElementsByClassName('range-revamp-product-identifier__number')[0].innerText
    ...
    fetch(searchUri).then(function (result) { return result.json() })
      .then(function (result) {
        const otherPrice = ' €' + result.searchResultPage.productWindow[0].priceNumeral
        ...
        priceSpan[0].lastChild.style.color = '#aaa'

        const divToAppend = document.getElementsByClassName('range-revamp-pip-price-package__price-wrapper')[0]
